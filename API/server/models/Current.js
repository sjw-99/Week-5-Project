    ...
    static async addQuestionToCurrent(question_id) {
        ...
        if(current_mission.rows.length < 10) {
            let response = await db.query(
                    'INSERT INTO current (question_intro, question,student_option,correct_option,topic) SELECT question_intro, question, correct_option,correct_option, topic FROM question WHERE question_id = $1;',[question_id]);
        } else {
